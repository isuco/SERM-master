...
def _create_scheduler(optimizer,opt):
    warmup_steps = 500
    if opt['lr_schedule'] == "warmup_linear":
        return get_linear_schedule_with_warmup(optimizer, warmup_steps, opt['num_train_steps'])
    if opt['lr_schedule'] == "warmup_constant":
        return get_constant_schedule_with_warmup(optimizer, warmup_steps)

    raise RuntimeError("Unsupported scheduler: " + opt['lr_schedule'])


def change_lr(optimizer, decay):
